...
export class BitmexClient {
  ...
  constructor(private onUpdate: (data: any) => void) {
    setInterval(() => this.pruneOldTrades(), 1000); // todo: Allow for this to be stopped
    this.setupWebsocket();
  }
  ...
  private onMessage(e: { data: string }) {
    ...
    if (table === "trade" && action === "insert") {
      ...
      this.addTrade(trade);
      this.onUpdate(this.getData()); // todo: throttle this
    }
  }
    ...
    function prune(arr: Array<[number, number]>) {
      for (let i = 0; i < arr.length; ) {
        ...
        if (timestamp > oldestAllowedAge) {
          i++; // Could get away with `break`ing here. Safer to iterate over entire array
        } else {
          arr.splice(i, 1);
        }
      }
